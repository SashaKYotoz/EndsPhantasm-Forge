...
public class ConfiguredData {
    ...
    public static void registerClient() {
        register(Phantasm.makeID("models/item/crystalline_knife.json"),
                () -> ModList.get().isLoaded("kitchenprojectiles") && Phantasm.isFarmersDelightLoaded(),
    ...
    private static class Common {
        public static String changeBiomeSource(JsonElement json) {
            ...
            if (json.getAsJsonObject().get("generator")
                ...
                for (JsonElement e : entries) {
                    if (e.getAsJsonObject().get("biome").getAsString().equals(Biomes.END_HIGHLANDS.location().toString())) {
                ...
                if (highlands != null && customCount > 0) {
                    ...
                    double customStep = 0;
                    for (Pair<ResourceLocation, Double> entry : EndDataCompat.getEnabledWeightedBiomes())
                    ...
                    int j = 1;
                    for (Pair<ResourceLocation, Double> entry : EndDataCompat.getEnabledWeightedBiomes()) {
                        ResourceLocation biome = entry.getFirst();
                        ...
                        if (j != customCount || customCount == 1) {
                            max += highlandsStep;
                            endEntries.add(EndDataCompat.splitHypercube(Biomes.END_HIGHLANDS.location(), highlands, noise, min, max));
                            min = max;
                        }
        ...
        public static String changeNoiseRouter(JsonElement json) {
            ...
            for (Pair<ResourceLocation, JsonObject> biome : EndDataCompat.getCustomRules()) {
